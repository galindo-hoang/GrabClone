...
//    private val src = Point.fromLngLat(stimulateViewModel.source?.long!!,stimulateViewModel.source?.lat!!)
//    private val des = Point.fromLngLat(stimulateViewModel.destination?.long!!,stimulateViewModel.destination?.lat!!)
    val src = Point.fromLngLat(106.665290,10.838678)
    val des = Point.fromLngLat(106.680264,10.799194)
    @Inject
...
//            }
    ...
    override fun onStart() {
        ...
        mapboxReplayer.registerObserver{
            // problem when refresh token expired
            runBlocking(Dispatchers.IO) {
                val replayEventUpdateLocation = it[0] as ReplayEventUpdateLocation
                sendCurrentLocationService.invoke(
                    LatLong(
                        replayEventUpdateLocation.location.lat,
                        replayEventUpdateLocation.location.lon
                    )
                )
                if(replayEventUpdateLocation.location.speed == 0.0){
    ...
    private lateinit var mDoneDrivingDialog: Dialog
    private fun showDoneDrivingDialog(){
        this.mDoneDrivingDialog = Dialog(this)
        this.mDoneDrivingDialog.setContentView(R.layout.dialog_done_driving)
        this.mDoneDrivingDialog.findViewById<AppCompatButton>(R.id.btn_done_driving).setOnClickListener {
            stimulateViewModel.doneDriving()
            this.mDoneDrivingDialog.dismiss()
        }
    }
